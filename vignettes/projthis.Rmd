...
-   **workflow (directory)**: a directory containing a sequence of RMarkdown files and a `data/` subdirectory.
    ...
    A workflow should not be nested in a subdirectory of any other workflow, they should have independent paths.
...
For those functions that take a `path` or a `name` as an argument, I try to be clear and consistent in the use of this vocabulary.

### Managing a workflow

To create a workflow directory, call `proj_use_workflow()` from somewhere the within the project directory.
It accepts a `path` argument, which is relative to the project directory.
